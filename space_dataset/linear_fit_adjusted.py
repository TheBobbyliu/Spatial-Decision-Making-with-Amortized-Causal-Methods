...
def adjusted_r2_score(y_true: np.ndarray, y_pred: np.ndarray, num_predictors: int) -> float:
    """Compute adjusted R^2 for validation sets with p predictors."""
    base_r2 = r2_score(y_true, y_pred)
    n = len(y_true)
    p = num_predictors
    if n <= p + 1:
        return base_r2
    return 1 - (1 - base_r2) * (n - 1) / (n - p - 1)
...
def fit_linear_regression(
    ...
    adj_r2 = adjusted_r2_score(y[val_idx], preds, X.shape[1])
    return model, preds, mae, adj_r2
...
class MultiTaskLinearEmbeddingModel(torch.nn.Module):
    def __init__(
        ...
        self.classification_head = (
            torch.nn.Linear(embedding_dim, classification_targets)
            if classification_targets > 0
            else None
        )
    ...
    def forward(
        ...
        cls_out = self.classification_head(neck) if self.classification_head else None
        # reg_out = self.regression_head(z)
        # cls_out = self.classification_head(z) if self.classification_head else None
...
def train_linear_embedding(
    ...
    with torch.no_grad():
        ...
        val_r2 = adjusted_r2_score(
            y_val.cpu().numpy().ravel(),
            val_preds.cpu().numpy().ravel(),
            features.shape[1],
        )
        all_embeddings = (
...
def train_multitask_embedding(
    ...
    # regression_targets = np.concatenate(
    #     [covariate_targets, outcome.reshape(-1, 1)], axis=1
    # )
    ...
    for _ in range(epochs):
        ...
        for i in range(0, X_train.shape[0], batch_size):
            batch_idx = indices[i:i + batch_size]
            optimizer.zero_grad()
            reg_preds, cls_preds, _ = model(X_train[batch_idx])
            loss = mse_loss(reg_preds, y_reg_train[batch_idx])
            if cls_preds is not None:
                loss = loss + bce_loss(cls_preds, y_cls_train[batch_idx])
            loss.backward()
            optimizer.step()
            losses.append(loss.item())
        train_loss_history.append(float(np.mean(losses) if losses else 0.0))
...
def fit_and_score_cate(
    ...
    r2 = adjusted_r2_score(y_val, y_pred, X.shape[1])
    cate = mu1 - mu0
...
def run_cate_masking_experiments(
    ...
    for idx, res in enumerate(schedule):
        ...
        metrics = (
            f"{label_prefix} {res.label:<18} -> MAE={mae:.4f} | Adj R^2={r2:.4f} | "
...
def main() -> None:
    ...
    print("\nExperiments (validation MAE | Adj R^2):")
    ...
    for res in cov_results:
        print(
            ...
            f"MAE={res.mae:.4f} | Adj R^2={res.r2:.4f} | "
    ...
    print(
        ...
        f"MAE={outcome_result.mae:.4f} | Adj R^2={outcome_result.r2:.4f}"
    )

    print("\n[Features -> Covariates]")
    for cov in covariates:
        res = features_to_targets(features, df[cov], train_idx, val_idx, f"features -> {cov}")
        print(f"{cov:<12} MAE={res.mae:.4f} | Adj R^2={res.r2:.4f}")
    ...
    if args.pca:
        ...
        print(
            f"Embedding model validation -> MAE={embed_val_mae:.4f} | Adj R^2={embed_val_r2:.4f} "
    ...
    #     )
    #     print(
    #         f"Embedding -> {cov} -> MAE={mae:.4f} | Adj R^2={r2:.4f} "
    #     )
